...
-- V6: Fixed column name error - plan_id instead of allocation_plan_id
--     Simplified: counts ALL shipments in period (no plan filter needed)
...
BEGIN
  ...
  -- Count each shipment only ONCE based on origin_node_id
...
V6: Fixed column name error (plan_id not allocation_plan_id). Counts all shipments in period.
Each shipment counted only once based on origin_node_id for origin cities.';
