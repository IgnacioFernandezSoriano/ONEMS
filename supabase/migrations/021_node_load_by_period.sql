...
BEGIN
  ...
  RETURN QUERY
  WITH 
  -- Get all weeks in the period (only weeks that overlap with the date range)
  week_series AS (
    SELECT 
      generate_series(
  ...
  -- Get shipments in the period grouped by node and week
  shipments_by_week AS (
    ...
    CROSS JOIN nodes n
    LEFT JOIN material_shipments s ON 
      s.node_id = n.id AND
      s.account_id = p_account_id AND
      s.created_at::date >= GREATEST(w.week_start, p_start_date) AND
      s.created_at::date <= LEAST(w.week_end, p_end_date)
    LEFT JOIN cities c ON n.city_id = c.id
    WHERE n.account_id = p_account_id
      AND n.city_id IS NOT NULL
    GROUP BY n.id, n.node_code, c.name, c.id, w.week_num, w.week_start, w.week_end
  ),
  ...
  city_stats AS (
    SELECT 
      city_id,
      week_num,
      AVG(shipment_count) AS weekly_avg,
      STDDEV(shipment_count) AS weekly_stddev,
      SUM(shipment_count) AS weekly_total,
      COUNT(DISTINCT node_id) AS node_count
    FROM shipments_by_week
    GROUP BY city_id, week_num
  ),
  city_period_stats AS (
    SELECT 
      city_id,
      AVG(shipment_count) AS period_avg,
      STDDEV(shipment_count) AS period_stddev,
      SUM(shipment_count) AS period_total,
      COUNT(DISTINCT node_id) AS node_count
    FROM shipments_by_week
    GROUP BY city_id
  )
...
'Analyzes node load distribution for a custom date period with configurable reference load and deviation thresholds. 
Calculates weekly shipment counts, city averages, and classifies nodes as normal, high load, or saturated based on the criteria provided.';
