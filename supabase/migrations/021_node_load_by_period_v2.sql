...
BEGIN
  ...
  shipments_by_week AS (
    ...
    CROSS JOIN nodes n
    LEFT JOIN panelists p ON p.node_id = n.id AND p.account_id = p_account_id
    LEFT JOIN material_shipments s ON 
      s.panelist_id = p.id AND
...
CORRECTED: Uses proper table structure with panelists.node_id and nodes.auto_id';
