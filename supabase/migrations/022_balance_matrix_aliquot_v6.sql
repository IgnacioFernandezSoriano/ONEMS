...
-- V6: Synchronized date filters with rpc_get_node_load_by_period using GREATEST/LEAST
-- V5: Fixed double counting - now counts only by origin_node_id
  ...
  WHILE v_iteration < v_max_iterations LOOP
    ...
    LOOP
      ...
      FROM (
        ...
        WHERE apd.origin_node_id = v_from_cell.node_id
      ...
      IF v_shipments_to_move IS NOT NULL AND array_length(v_shipments_to_move, 1) > 0 THEN
        ...
        IF p_apply_changes THEN
          ...
          LOOP
            ...
            SET 
              origin_node_id = v_to_cell.node_id,
