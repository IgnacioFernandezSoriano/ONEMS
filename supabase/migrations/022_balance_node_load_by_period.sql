...
BEGIN
  ...
  WITH node_loads AS (
    ...
    WHERE node_id IN (
      SELECT id FROM nodes WHERE city_id = p_city_id
    )
  ...
  FOR v_from_node IN
    WITH node_loads AS (
      SELECT 
        n.id AS node_id,
        n.node_code,
        COUNT(s.id) AS current_load,
        AVG(COUNT(s.id)) OVER () AS avg_load
      FROM nodes n
      LEFT JOIN material_shipments s ON 
        s.node_id = n.id AND
        s.created_at::date BETWEEN p_start_date AND p_end_date AND
        s.status = 'pending'
  ...
  LOOP
    ...
    GROUP BY n.id, n.node_code
    HAVING COUNT(s.id) < v_from_node.avg_load * 0.9  -- 10% below average
    ...
    IF v_to_node.node_id IS NOT NULL THEN
      ...
      FROM (
        SELECT id
        FROM material_shipments
        WHERE node_id = v_from_node.node_id
          AND created_at::date BETWEEN p_start_date AND p_end_date
          AND status = 'pending'
        ORDER BY created_at
      ...
      IF p_apply_changes AND v_shipments_to_move IS NOT NULL THEN
        ...
        LOOP
          ...
          SET 
            node_id = v_to_node.node_id,
            notes = COALESCE(notes || ' ', '') || '[Auto-balanced]'
...
'Balances node load distribution for a custom date period by moving pending shipments from overloaded nodes to underloaded nodes within the same city.';
