...
-- V5: Fixed double counting - each shipment counted only once based on city context
--     Added filter for active/applied allocation plans only
...
BEGIN
  ...
  shipments_by_week AS (
    SELECT 
      ...
      w.week_start,
      w.week_end,
      -- For ORIGIN city: count shipments where this node is origin
      COUNT(DISTINCT CASE 
        WHEN apd.origin_node_id = n.id 
          AND apd.origin_city_id = c.id
        THEN apd.id 
      ...
      COUNT(DISTINCT CASE 
        ...
        THEN apd.id 
      END) AS sent_count,
      -- Received: where node is destination (for destination cities)
      COUNT(DISTINCT CASE 
        WHEN apd.destination_node_id = n.id 
          AND apd.destination_city_id = c.id
        THEN apd.id 
    ...
    LEFT JOIN allocation_plan_details apd ON 
      ...
      apd.allocation_plan_id IN (SELECT id FROM active_plans) AND
...
V5: Fixed double counting - each shipment counted only once based on city context (origin_node_id for origin cities).
Added filter for active/applied allocation plans only.';
