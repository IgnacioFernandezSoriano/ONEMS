...
-- V4: Fixed logic - classifies nodes based on PERIOD AVERAGE, not individual weeks
...
BEGIN
  ...
  shipments_by_week AS (
    SELECT 
      ...
      w.week_start,
      w.week_end,
      -- Total shipments (sent + received) - NO filter by status
      COUNT(DISTINCT apd.id) AS shipment_count,
      -- Sent: where node is origin
      COUNT(DISTINCT CASE 
        WHEN apd.origin_node_id = n.id THEN apd.id 
      END) AS sent_count,
      -- Received: where node is destination
      COUNT(DISTINCT CASE 
        WHEN apd.destination_node_id = n.id THEN apd.id 
...
V4: Fixed classification logic - nodes are classified based on their PERIOD AVERAGE, not individual weeks.
This ensures a node has only ONE status (normal/high/saturated) for the entire period.';
