-- Drop existing slas table if exists
DROP TABLE IF EXISTS slas CASCADE;

-- Create simplified SLAs table for Network Diagnostics Module
...
CREATE TABLE slas (
  ...
  -- No specific readers, just center-level
  ...
  CONSTRAINT slas_operational_fields_check CHECK (
    (sla_type = 'operational' AND postal_center_id IS NOT NULL AND from_postal_center_id IS NULL AND to_postal_center_id IS NULL)
    OR
    (sla_type = 'distribution' AND from_postal_center_id IS NOT NULL AND to_postal_center_id IS NOT NULL AND postal_center_id IS NULL)
  ),
  ...
  -- Unique constraint for distribution SLAs
...
CREATE INDEX idx_slas_operational ON slas(account_id, postal_center_id) WHERE sla_type = 'operational';

-- Unique index for operational SLAs (partial index)
CREATE UNIQUE INDEX unique_operational_sla ON slas(account_id, postal_center_id) WHERE sla_type = 'operational';
CREATE INDEX idx_slas_distribution ON slas(account_id, from_postal_center_id, to_postal_center_id) WHERE sla_type = 'distribution';
...
-- Comments
COMMENT ON TABLE slas IS 'Service Level Agreements for Network Diagnostics - defines expected times for operational (Entry→Exit within center) and distribution (between centers) routes';
COMMENT ON COLUMN slas.sla_type IS 'Type of SLA: operational (Entry→Exit within center) or distribution (between centers)';
COMMENT ON COLUMN slas.postal_center_id IS 'For operational SLAs: the postal center (Entry→Exit)';
COMMENT ON COLUMN slas.from_postal_center_id IS 'For distribution SLAs: origin postal center';
COMMENT ON COLUMN slas.to_postal_center_id IS 'For distribution SLAs: destination postal center';
